    ...
    if Service.get_user_lan(TgUser.objects.filter(tg_id=message.from_user.id)) == 'oz':
        if message.text == LAN[Service.get_user_lan(TgUser.objects.filter(tg_id=message.from_user.id))]['kelishilgan']:
            for ioz in UserCart.objects.filter(user__tg_id=message.from_user.id, status_check=False, status=False):
                # bot.send_message(message.from_user.id, f'Kiritgan ma\'lumotlaringiz to\'g\'rimi?\n\nKategoriya: {ioz.category.oz}\nPodkategoriya: {ioz.podcategory.oz}\nBoshlang\'ich narx: "Kelishilgan"\nMoshina rusumi: {ioz.avto.oz}\nMoshina sig\'imi: {ioz.kub}\nViloyat: {ioz.viloyat.oz}\nTuman: {ioz.tuman.oz}\nTelefon raqam: {ioz.telefon}\nTelegram: @{ioz.username}\nTarif (paket): {ioz.paket.name_oz}\n', reply_markup=done_button)

                cat = ioz.category.oz
                nar = LAN[Service.get_user_lan(TgUser.objects.filter(tg_id=message.from_user.id))]['kelishilgan'][2:]
            ...
            done_button = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
            done_button.add(types.KeyboardButton(text=LAN[Service.get_user_lan(TgUser.objects.filter(tg_id=message.from_user.id))]['yes']), types.KeyboardButton(text=LAN[Service.get_user_lan(TgUser.objects.filter(tg_id=message.from_user.id))]['no']))
            bot.send_message(message.from_user.id, f'Kiritgan ma\'lumotlaringiz to\'g\'rimi?\n\nKategoriya: {cat}\nPodkategoriya: {podcat}\nBoshlang\'ich narx: {nar}\nMoshina rusumi: {avto}\nMoshina sig\'imi: {kub}\nViloyat: {viloyat}\nTuman: {tuman}\nTelefon raqam: {telefon}\nTelegram: @{username}\nTarif (paket): {paket}\n', reply_markup=done_button)
